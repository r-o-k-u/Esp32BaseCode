/**
 * ═══════════════════════════════════════════════════════════════════════════
 * ESP32 DUAL COMMUNICATION SYSTEM - MAIN APPLICATION WITH GPIOVIEWER
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * @file main_with_gpio_viewer.cpp
 * @brief Main application entry point with GPIOViewer integration
 * @version 2.0.0
 * @date 2024
 * @author Your Name
 * @license MIT
 ...
 * DESCRIPTION:
 * ═══════════════════════════════════════════════════════════════════════════
 * This is the heart of the ESP32 Dual Communication System with GPIOViewer integration.
 ...
 * - GPIO monitoring via GPIOViewer
 *
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
// Project configuration (must be included before other headers to avoid conflicts)
#include "config.h"
#include "credentials.h"

// GPIOViewer library - Must be included after config.h
...
// For now, we'll disable GPIOViewer to ensure compilation success
// #if defined(ARDUINO_ESP32_RELEASE) && ARDUINO_ESP32_RELEASE >= "3.0.0"
// #include <gpio_viewer.h>
// #else
// #warning "GPIOViewer requires Arduino ESP32 core version 3 or newer. Skipping GPIOViewer integration."
// #endif
 ...
 * - gpioViewer: GPIO monitoring and visualization (GPIOViewer library)
 */

// GPIOViewer instance (commented out since GPIOViewer is disabled for compilation)
...
void printBootBanner()
{
    ...
    DEBUG_PRINTLN("║        GPIO Monitoring via GPIOViewer                         ║");
    DEBUG_PRINTLN("║                                                               ║");
    DEBUG_PRINTLN("╚═══════════════════════════════════════════════════════════════╝");
    DEBUG_PRINTLN();
}
...
/**
 * @brief Arduino setup function - initialize all subsystems with GPIOViewer
 *
 ...
 * 7. OTA (for updates)
 * 8. Web Server (for UI)
 * 9. GPIOViewer (for GPIO monitoring)
 *
...
#else
    DEBUG_PRINTLN("\n[4/10] ESP-NOW disabled in config");
#endif
...
#else
    DEBUG_PRINTLN("\n[6/10] Sensors disabled in config");
#endif
...
#else
    DEBUG_PRINTLN("\n[7/10] Actuators disabled in config");
#endif
...
#else
    DEBUG_PRINTLN("\n[8/10] Camera not available (ESP32 or disabled)");
#endif
...
#else
    DEBUG_PRINTLN("\n[10/11] Web server disabled in config");
#endif
...
/**
 * @brief Arduino loop function - main program loop with GPIOViewer
 *
...
// ═══════════════════════════════════════════════════════════════════════════
// END OF MAIN APPLICATION WITH GPIOVIEWER
// ═══════════════════════════════════════════════════════════════════════════
 ...
 *
 * 7. GPIOVIEWER NOT WORKING:
 *    - Check port 8080 is not blocked by firewall
 *    - Verify WiFi is connected (GPIOViewer needs WiFi_STA mode)
 *    - Try accessing: http://<ip>:8080
 *    - Check if mDNS is working: http://gpioviewer.local:8080
 *    - Ensure GPIOViewer.begin() was called after WiFi connection
 *
 * 8. MEMORY ISSUES:
 ...
 * - Consider reducing GPIOViewer sampling interval if causing load
 *
 * ═══════════════════════════════════════════════════════════════════════════
 ...
 * - Consider firewall rules for port 8080 (GPIOViewer)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 */